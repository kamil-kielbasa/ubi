...
UBI-only library numbers (partition size vs. code size) are documented in {doc}`/architecture/plain_architecture`: the DeviceTree `ubi_partition` used in this repo is **128 KiB** (STM32U5) / **64 KiB** (nRF5340); the measured **library** ROM footprint is **~9.5 KB** plain / **~28.6 KB** secure (Cortex-M33, `-Os`, library archive only — PSA Crypto/mbedTLS not counted).
