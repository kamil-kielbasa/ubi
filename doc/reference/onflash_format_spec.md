...
- **Anchor rewrite.** `ubi_secure_anchor_rewrite_for_dirty_witness()` advances the cache (with the same pre-mutation rule) when refreshing the anchor for a sole-witness dirty PEB.
