...
mock_reject_freshness(const struct ubi_secure_freshness *freshness, void *user_data)
{
	(void)freshness;
	(void)user_data;
 ...
 *          secure_cfg. The backend detects blank media and formats in secure mode.
 *
 ...
 *
 * \details Scenario: Format a device as plain (secure_cfg == NULL), then attempt
 *          to re-init with a secure secure_cfg. The backend must reject
 ...
 *          secure_cfg == NULL. The plain backend must reject the non-standard
 *          on-flash magic.
 *
 ...
 *          callback that always returns UBI_SECURE_ROLLBACK_REJECT.
 *
 ...
 *
 * \details Scenario: Pass a secure_cfg with get_key_id set to NULL.
 *
 ...
 *
 * \details Scenario: Pass a secure_cfg with allowed_key_versions_len == 0.
 *
 * \expect ubi_device_init returns -EINVAL, device handle is NULL.
 */
 ...
 *
 * \details Scenario: Pass a secure_cfg whose requested_write_key_version (99)
 ...
 *          ambiguity in operator-visible state.  `validate_secure_cfg`
