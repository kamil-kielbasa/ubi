...
/* Include guard ------------------------------------------------------------------------------- */
#ifndef UBI_RES_PEB_H
#define UBI_RES_PEB_H
...
#define UBI_RES_PEB_NR_ACTIVE (2)

/* Types and type definitions ------------------------------------------------------------------ */

/** Classification of a reserved PEB after scanning. */
enum ubi_res_peb_state {
	UBI_RES_PEB_STATE_CORRUPT = 0, /**< Invalid data (bad magic or CRC). */
	UBI_RES_PEB_STATE_SPARE, /**< Erased/empty (all 0xFF), cold spare. */
	UBI_RES_PEB_STATE_ACTIVE, /**< Valid device header (correct magic + CRC). */
};
...
struct ubi_res_peb_scan {
	 ...
	 */
	enum ubi_res_peb_state state[UBI_DEV_HDR_NR_OF_RES_PEBS];
 ...
 */
int ubi_res_peb_overwrite(const struct ubi_mtd *mtd, const uint8_t *content, size_t content_len);

/**
 * \brief Commit updated headers to reserved PEBs and verify.
 *
 * Calls ubi_res_peb_overwrite() and then verifies the result via a
 * fresh scan.
 *
 * \param[in] mtd	UBI MTD device structure.
 ...
 * \return 0 on success, negative error code on failure.
 */
int ubi_res_peb_commit(const struct ubi_mtd *mtd, const uint8_t *content, size_t content_len);
 ...
 * \return 0 on success, negative error code on failure.
 */
int ubi_res_peb_read_content(const struct ubi_mtd *mtd, size_t peb_idx, uint8_t *content,
			     size_t content_len);
