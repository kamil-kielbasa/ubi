 ...
 *          secure_cfg == NULL to ubi_device_init() need not include this header.
 *
...
/* Plain-mode public surface: applications building with CONFIG_UBI_SECURE=n
 * still include this header (for example to pass secure_cfg=NULL to
...
/**
 * \defgroup ubi_secure_types UBI Crypto Types
...
enum ubi_secure_rollback_verdict {
	UBI_SECURE_ROLLBACK_ACCEPT = 0, /*!< Freshness acceptable — proceed with attach. */
	UBI_SECURE_ROLLBACK_REJECT = 1, /*!< Rollback detected — abort attach. */
};
...
enum ubi_secure_event_verdict {
	UBI_SECURE_EVENT_CONTINUE = 0, /*!< Continue normal operation. */
	UBI_SECURE_EVENT_ENTER_READ_ONLY = 1, /*!< Enter read-only mode for the session. */
};
 ...
 * payload in the ubi_secure_event union.
 */
enum ubi_secure_event_type {
	UBI_SECURE_EVENT_AUTH_FAILURE = 0, /*!< AEAD authentication failed on a
	                                                    PEB header or data record. */
	UBI_SECURE_EVENT_FORMAT_VIOLATION = 1, /*!< On-flash structure has valid auth
	                                                    tag but violates format rules. */
	UBI_SECURE_EVENT_KEY_VERSION_NOT_ALLOWLISTED = 2, /*!< Authenticated object carries a
	                                                       key version absent from the
	                                                       policy allowlist. */
	UBI_SECURE_EVENT_KEY_VERSION_UNAVAILABLE = 3, /*!< get_key_id callback failed —
	                                                    key material not accessible. */
	UBI_SECURE_EVENT_ROLLBACK_POLICY_MISMATCH = 4, /*!< On-flash freshness is behind the
	                                                    trusted reference (rollback). */
	UBI_SECURE_EVENT_FRESHNESS_SYNC_FAILURE = 5, /*!< sync_freshness callback returned
	                                                    a non-zero error code. */
	UBI_SECURE_EVENT_RNG_FAILURE = 6, /*!< Platform RNG could not produce
	                                                    a fresh salt for AEAD nonce. */
	UBI_SECURE_EVENT_KEY_ROTATE_SOON = 7, /*!< Usage crossed the soft threshold
	                                                    (ROTATE_SOON_PCT) — prepare a
	                                                    replacement key. */
	UBI_SECURE_EVENT_KEY_ROTATE_NOW = 8, /*!< Usage crossed the hard threshold
	                                                    (ROTATE_NOW_PCT) — writes may be
	                                                    rejected without a new key. */
	UBI_SECURE_EVENT_KEY_RETIRABLE = 9, /*!< All on-flash objects for this key
...
struct ubi_secure_event {
	enum ubi_secure_event_type type; /*!< Event discriminator. */
	struct ubi_secure_freshness freshness; /*!< Freshness at event time. */
	union {
		struct {
 ...
 */
typedef int (*ubi_secure_get_key_id_cb_t)(uint8_t key_version, psa_key_id_t *key_id_out);
 ...
 * \param[in] user_data  Opaque pointer from ubi_secure_config.user_data.
 *
 * \return Verdict: accept or reject.
 */
typedef enum ubi_secure_rollback_verdict (*ubi_secure_check_freshness_cb_t)(
	const struct ubi_secure_freshness *freshness, void *user_data);

/**
 * \brief Sync freshness after a commit.
 *
 * Called after each mutation commit (or every N mutations per
 * CONFIG_UBI_SECURE_FRESHNESS_SYNC_DELTA). The application should persist
 ...
 */
typedef int (*ubi_secure_sync_freshness_cb_t)(const struct ubi_secure_freshness *freshness,
					      void *user_data);
 ...
 */
typedef enum ubi_secure_event_verdict (*ubi_secure_event_cb_t)(const struct ubi_secure_event *event,
							       void *user_data);
 ...
 *
 * Passed as the secure_cfg argument to ubi_device_init(). When non-NULL,
 * the secure backend is selected. All callback pointers must be non-NULL.
 */
struct ubi_secure_config {
	struct ubi_secure_policy policy; /*!< Key policy for this session. */

	ubi_secure_get_key_id_cb_t get_key_id; /*!< Key-ID resolver. */
	ubi_secure_check_freshness_cb_t check_freshness; /*!< Attach-time freshness check. */
	ubi_secure_sync_freshness_cb_t sync_freshness; /*!< Post-commit freshness sync. */
	ubi_secure_event_cb_t event_cb; /*!< Security event handler. */
